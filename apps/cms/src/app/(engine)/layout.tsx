...
export default function EngineRootLayout({
  ...
  return (
    ...
    >
      <body
        className={cn(
          "min-h-screen bg-background font-sans antialiased text-foreground transition-colors duration-300 overflow-hidden"
        )}
      >
        <ThemeProvider
          attribute="class"
        ...
        >
          <SidebarProvider style={{"--sidebar-width": "16rem"} as React.CSSProperties}>
             {/* Header - Fixed Top */}
             <header className="fixed top-0 left-0 right-0 z-50 flex h-[52px] w-full items-center justify-between border-b border-border bg-background/80 px-4 backdrop-blur-md">
             ...
             {/* Sidebar - Pushed down by header height */}
             <AppSidebar className="top-[52px] !h-[calc(100svh-52px)]" />

             {/* Main Content - Pushed down by header height */}
             <SidebarInset className="mt-[52px] flex flex-col flex-1 overflow-hidden">
                <div 
